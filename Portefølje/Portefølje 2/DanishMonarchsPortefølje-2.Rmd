---
title: "Danish Kings"
author: "Ronja Egballe, Ida Frederiksen og Sara Marie Pedersen"
date: "2025-07-03"
...
kings1 <- read.csv("danske konger.csv")

kings2 <- read_csv("danske konger.csv", na = "NULL")

kings3 <- read.csv2("danske konger.csv")

kings4 <- read_csv2( "danske konger.csv", na = "NA")
...
kings_mid_year <- kings %>% 
  mutate(Duration = `Regerings slut`- `Regerings start`) %>% 
  mutate(Midt_år = `Regerings start` + Duration/2) %>% 
  glimpse()
```
...
kings_mid_year %>% 
  ggplot( aes(x = Midt_år, y = Duration)) +
  geom_point() +
  geom_smooth() +
  theme_bw()
...
```{r}
# YOUR CODE
kings_mid_year_challenge <- kings_mid_year %>%
  mutate(Midt_år = `Regerings start` + (`Regerings slut` - `Regerings start`) / 2)

library(ggplot2)

ggplot(kings_mid_year_challenge, aes(x = Midt_år, y = Duration)) +
  geom_point() +
