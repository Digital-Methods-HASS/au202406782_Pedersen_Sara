...
- first we see the longitud because it sow the East and west coordinates, and after that the lattitude, that shows the north and south coordinates
...
- it changes your box in the top rigth corner, when putting in the T it hides your 3 choiches in the box, and when you want to see them you simply just click the box, when putting in the F your choiches just shows your 3 choiches in the top right corner without hiding it 
...
Aarhus <- leaflet() %>% 
  setView(10.20583,56.15383, zoom = 13) %>% 
  addTiles()

esri <- grep("^Esri", providers, value = TRUE)

for (provider in esri) {
  Aarhus <- Aarhus %>% addProviderTiles(provider, group = provider)
}
...
# select the correct worksheet name

glimpse(places)  
# Question 3: are the Latitude and Longitude columns present? 
# Do they contain numeric decimal degrees?
- Yes, both Lattitude and Longtitude columns are present, and they both contain numeric decimal degrees
...
saveWidget(Studentmapp,"Studentmapp.html", selfcontained = TRUE )
...
# Your brief answer
- The cluster function is useful when you want to see areas with activities, but without specific details. If you don’t use the cluster function, you can view more specific locations of activities."
...
DANmap %>% 
  ...
  saveWidget(DANmap,"DAN.map.html", selfcontained = TRUE )
